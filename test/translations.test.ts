...
import { SnippetTranslator, SnippetParameters } from '../lib';
...
let compileDirectory: string = '';

// Prepare a compilation directory with the right dependencies in...
beforeAll(async () => {
  compileDirectory = await fs.promises.mkdtemp(path.join(tmpdir(), 'jsii-rosetta-translations-test-'));
  const nodeModulesDir = path.resolve(compileDirectory, 'node_modules');

  for (const tarball of await fs.promises.readdir(fixturesDir)) {
    if (!tarball.endsWith('.tgz')) {
      continue;
    }

    const file = path.join(fixturesDir, tarball);

    const workDir = await fs.promises.mkdtemp(path.join(tmpdir(), tarball));
    try {
...
afterAll(async () => {
  if (compileDirectory) {
    await fs.promises.rm(compileDirectory, { force: true, recursive: true });
  }
  compileDirectory = '';
});
  ...
  describe(`Translating ${path.relative(translationsRoot, typeScriptTest)}`, () => {
    ...
    beforeAll(() => {
      translator = new SnippetTranslator({
        ...
        parameters: { [SnippetParameters.$COMPILATION_DIRECTORY]: compileDirectory },
      });
    });
